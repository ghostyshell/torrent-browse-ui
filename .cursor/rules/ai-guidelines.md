...
## Runtime env injection — never expose real URLs in the repo

This repo is public on the `alt` remote. **Never write real infrastructure URLs** (backend API, database, etc.) into any file that is committed and pushed.

- **`.env.production`** must keep `https://your-api-host.example.com` as a placeholder. The real URL is injected at container startup by `/docker-entrypoint.d/40-env-config.sh` on the nginx server (server-side only, never committed).
- All frontend code reads the backend URL from `src/config/env.ts`, which checks `window.__ENV__` (set by the startup script) first, then falls back to `process.env.REACT_APP_*` (baked at build time).
- If you need to update the real backend URL, SSH into the nginx server and edit `/docker-entrypoint.d/40-env-config.sh`. Do not put it in `.env.production` or any committed file.
- **Always deploy via `sh scripts/deploy-frontend.sh`** — it builds, uploads, and re-runs the startup script so `env-config.js` is never left as the empty placeholder. Never deploy by running `npm run build` + manual tar/scp alone.
- Same rule applies to `torrent-search-go` (the Go backend) — the public alt remote must not contain real service URLs, secrets, or credentials. Those live in server env vars or sliplane dashboard config only.
