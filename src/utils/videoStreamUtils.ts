 ...
 * Updated to work with Plyr player
 */
...
export const requiresCorsHandling = (url: string): boolean => {
  ...
  try {
    ...
    // Also check for Real-Debrid CDN patterns
    const realDebridPatterns = [
      /.*\.rdb\.io$/,
      /.*\.real-debrid\.com$/,
      /.*downloads\.real-debrid\.com$/,
    ];

    return (
      noCorsServices.some((service) => hostname.includes(service)) ||
      realDebridPatterns.some((pattern) => pattern.test(hostname))
    );
  } catch {
    return false;
  }
...
/**
 * Plyr specific utilities
 */

/**
 * Get optimal Plyr player options for streaming services
 */
